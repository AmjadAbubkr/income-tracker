...
export async function exportToExcel(
  ...
  filteredEntries.forEach((entry) => {
    ...
    salesData.push([
      ...
      formatCurrency(product?.priceMinor || 0, currency),
      entry.quantity,
      formatCurrency((product?.priceMinor || 0), currency),
  ...
  filteredEntries.forEach((entry) => {
    ...
    productPerformance[entry.productId].revenue += entry.amountMinor;
  ...
  Object.values(productPerformance).forEach((perf) => {
    performanceData.push([
      ...
      perf.items,
      formatCurrency(Math.round(perf.revenue / perf.sales), currency),
    ]);
  });
  ...
  if (period === 'monthly') {
    ...
    filteredEntries.forEach((entry) => {
      ...
      dailyBreakdown[entry.date].revenue += entry.amountMinor;
    ...
    Object.entries(dailyBreakdown)
      ...
      .forEach(([date, data]) => {
        dailyData.push([
          ...
          data.items,
          formatCurrency(Math.round(data.revenue / data.sales), currency),
        ]);
      });
...
function calculateAnalytics(
  ...
  const totalRevenue = entries.reduce((sum, e) => sum + e.amountMinor, 0);
  ...
  entries.forEach((entry) => {
    ...
    productStats[entry.productId].revenue += entry.amountMinor;
  });
